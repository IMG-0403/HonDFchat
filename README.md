...
PC版の画面では、実際のバーコードデータと希望データ出力結果を比較して、Data Format設定コマンドと設定用バーコードを生成できます。

- 対象コードは「全てのコード」「単体」「複数」から選択します。
- `[ENTER]`、`[TAB]`、`[CR]`、`[LF]`、`[CRLF]`、`[GS]` などは制御文字として扱います。
- `[TAB*2]` のように `*回数` を付けると、同じ制御文字を繰り返します。
- `{A->B}` は、読み取りデータ内の A を B へ置換する指定です。
- `[`、`{`、`\` を通常文字として扱う場合は、直前に `\` を付けます。

試験版は通常文字の挿入、制御文字の付加、1文字から1文字への置換に対応しています。元データの削除や並べ替えが必要な場合は生成を停止します。生成前にブラウザー内で出力をシミュレーションし、希望出力との一致を確認します。
...
- 自動生成: 複数の「N桁目からM桁」を連結して出力
- 自動生成: 任意のコード種で「N桁目以降」を出力
...
質問に「設定削除して」「設定削除してから」などが含まれる場合は、現在のデータフォーマット設定を削除してから登録するため、回答コマンドの先頭に `DFMDF3;` を付加します。
